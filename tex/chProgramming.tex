...
we casually use the \Coq{} term \C{nat} to refer to the collection of natural
...
assume implicitly that the \Coq{} symbols  \C{0}, \C{1}, \C{2}
represent natural numbers which represent the corresponding numerals
$0, 1, 2, \dots$.


\subsection{Defining functions, performing
...
The label \C{nat} is in fact called a {\em type}.
...
About f.
\end{coq-left}
\begin{coqout-right}%{run=about_f_run;dots}{title=Response,width=6.5cm}
f : nat -> nat
\end{coqout-right}
...
The output of this \C{Print} query has some similarities with the
 ...
 actual definition of \C{f} and its type information. Note that the way
...
For example the function \C{f} we just defined can \emph{only be
  applied to a natural number}, i.e., a term of type \C{nat}.
\Coq{} provides a command to check whether
an expression is \emph{well typed}:\index[concept]{well typed}
 as \C{3} is a natural number, the following query succeeds:
...
But \C{f} cannot be applied to an argument that is not a natural number,
...
\index[vernac]{\C{Compute}}

We can observe that in the course of the computation, argument \C{3} has been
...
command like \C{Compute} takes advantage of the
...
\C{bool} and \C{nat} and to the \Coq{} language constructs that
...
and \C{false}, representing $\top$ and $\bot$ respectively. The
...
\end{coq}
\coqrun{name=non_zero_run}{ssr,non_zero}
%
...
\C{p} have been replaced by \C{k}.  In our example, the value of
\C{k.+1} is \C{5}, thus the value of \C{k} is \C{4} and \C{t1} is \C{true}.
...
the \C{(addn n m)} program simply stacks \C{n} times the successor symbol
on top of \C{m}.
...
of applying \C{.+1} to one of the arguments. Conversely, comparing two
natural numbers is implemented by repeating the operation of fetching a
subterm. Consider two terms \C{m} and \C{n} representing the natural
numbers $m$ and $n$ respectively. Then $m$ is larger than $n$ if and only
if \C{m} has $k$ more constructors than \C{n} for some $k \geq
...
the predecessor (\C{predn}) and double (\C{double}) functions.
\index[coq]{\C{(_ .-1)}}
\index[coq]{\C{(_ .*2)}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \section{Container data types}\label{sec:poly}
...
to iterate contants other than \C{cons}.
For example, one can write  the boolean conjunction
of three terms as \C{[&& true, false & true]}, the 
...
\index[concept]{recursion}
Terms of type \C{(seq A)}, for a type \C{A}, are finite stacks of
...
carries a datum of type \C{A}. Here again, recursion provides a way
...
odd ones and replacing the even ones by the \C{None} default value:
...
first component: it can thus be obtained as \C{(3,true,4).1.2}. This is a
...
data type can be seen as the (Cartesian) product of the arguments.\footnote{In reality the data type of pairs, as it comes with \Coq{}, is called \C{prod} and its constructor \C{pair}.}  The
...
\cite[``Interpretation scopes'']{Coq:manual}).\index[concept]{notation scope}
...
As we mentioned in section~\ref{sec:functions}, the \C{Compute}
...
evaluation strategy.  One difference between the \C{Eval simpl}
...
number of  successor symbols \C{S}
...
name we may find more convenient, as in \C{(\\sum\_(1 <= j < n) j)},
...
variable \C{i} in the expression \C{F}, as mentioned in
...
% exhibit no problems: e.g., "Compute \sum_ (2 <= a < 6) a."
...
description of the \C{Notation} command goes beyond the scope of this
...
%   end.
% Compute is_empty None.
% Compute is_empty (Some true).
% (* Note: the function is polymorphic! *)
% Compute is_empty (Some 4).
% \end{coq}
%
...
%   end.
%
% Compute get_default None 3.
% Compute get_default (Some 4) 3.
% \end{coq}
%
% More containers
%
...
%
% (* what does this do? *)
% Compute fold cons (1 :: 2 :: 3 :: nil) nil.
% \end{coq}
%
